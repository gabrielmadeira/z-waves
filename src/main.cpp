...
int main()
{
    ...
    GLFWwindow* window;
    window = glfwCreateWindow(800, 800, "INF01047 - 322863 - Gabriel Madeira", NULL, NULL);
    if (!window)
    {
        glfwTerminate();
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        glm::vec4 camera_position_c  = glm::vec4(g_DeslocX,g_DeslocY,g_DeslocZ,1.0f); // Ponto "c", centro da câmera
        glm::vec4 camera_lookat_l    = glm::vec4(0.0f+g_DeslocX,0.0f+g_DeslocY,0.0f+g_DeslocZ,1.0f); // Ponto "l", para onde a câmera (look-at) estará sempre olhando
        glm::vec4 camera_view_vector = camera_position_c - (camera_position_c+glm::vec4(x,y,z,0.0f));; // Vetor "view", sentido para onde a câmera está virada
